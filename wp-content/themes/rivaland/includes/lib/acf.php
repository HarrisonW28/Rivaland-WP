...
if (function_exists('acf_add_options_page')) {
  ...
  // Also keep Site Options for backward compatibility
  acf_add_options_page([
 ...
 * Load ACF JSON
 */
add_filter('acf/settings/save_json', 'rivaland_acf_json_save_point');
function rivaland_acf_json_save_point($path) {
    $path = get_stylesheet_directory() . '/acf-json';
    return $path;
}

add_filter('acf/settings/load_json', 'rivaland_acf_json_load_point');
function rivaland_acf_json_load_point($paths) {
    unset($paths[0]);
