...
function acf_img($image = null, $classes = [], $size = 'full', $attributes = []) {
  if (!$image) return;

  $classes = (is_array($classes) && !empty($classes)) ? implode(' ', $classes) : $classes;
  
  // Get the attachment ID from the ACF image array
  ...
  if ($attachment_id) {
    ...
    // Use wp_get_attachment_image with proper attributes
    return wp_get_attachment_image($attachment_id, $size, false, $attributes);
  }

  // Fallback to direct URL if no attachment ID is found
  $class_str = ($classes) ? ' class="' . $classes . '"' : '';
  $attributes_str = '';
  
  // Add any additional attributes to the img tag
  foreach ($attributes as $key => $value) {
    if ($key !== 'class') { // Skip class as it's already handled
      $attributes_str .= ' ' . esc_attr($key) . '="' . esc_attr($value) . '"';
    }
  }

  return "
    <img src='{$image['url']}' alt='{$image['alt']}'{$class_str}{$attributes_str}>
  ";
}
